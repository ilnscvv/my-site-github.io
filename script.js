(function() {
  ...
  // Появление блоков при скролле (оставляем visible после первого появления)
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('visible');
      }
      // Убрали else, чтобы класс не удалялся при выходе из вьюпорта
    });
  }, { threshold: 0.15 });
...
// Подсветка активного пункта меню при скролле (Spy Scroll)
